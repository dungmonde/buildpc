...
<div class="min-h-screen bg-gray-950 text-white px-8 py-10">
    <div class="max-w-2xl mx-auto">
        ...
        <div class="mb-8">
            ...
            <h1 class="text-2xl font-bold text-white">Sửa linh kiện</h1>
        ...
        <form action="{{ route('admin.components.update', $component->id) }}" method="POST" class="bg-gray-900 border border-gray-800 rounded-2xl p-6 space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $component->name) }}"
                    class="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2.5 text-white placeholder-gray-500 focus:outline-none focus:border-blue-500 text-sm" required>
            </div>
            ...
            <div>
                ...
                <textarea name="specs" rows="4"
                    class="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2.5 text-white placeholder-gray-500 focus:outline-none focus:border-blue-500 text-sm resize-none">{{ old('specs', $component->specs ?? '') }}</textarea>
            </div>

            {{-- Giá hiện tại (chỉ hiển thị, không sửa ở đây) --}}
            <div class="bg-gray-800/50 border border-gray-700/50 rounded-xl p-4 flex items-center justify-between">
                <div>
                    <p class="text-xs text-gray-400 uppercase tracking-wider font-semibold">Giá hiện tại</p>
                    <p class="text-lg font-bold text-green-400 mt-0.5">{{ number_format($component->price, 0, ',', '.') }}₫</p>
                </div>
                <a href="{{ route('admin.components.price', $component->id) }}"
                   class="bg-green-600/20 hover:bg-green-600/40 border border-green-500/30 text-green-400 text-sm font-semibold px-4 py-2 rounded-lg transition-colors">
                    Cập nhật giá →
                </a>
            </div>
